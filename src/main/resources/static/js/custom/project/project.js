...
function loadData(pageIndex, pageSize) {
	$.ajax({
		type : "POST",
		url : '/projects/mng/list?pageIndex=' + pageIndex + '&pageSize=' + pageSize
				+ '&_csrf=' + csrf,
...
function openAddWindow() {
	openWindow("新增工程项目信息", '/projects/mng/formInit', 470, 385);
}
...
function openEditWindow(id) {
	var href = '/projects/mng/findOne?id=' + id;
	openWindow("编辑工程项目信息", href, 470, 385);
}
...
function doDelete(param) {
	$.ajax({
		type : "POST",
		url : '/projects/mng/delete?_csrf=' + csrf,
		data : param,
		success : afterDelete,
		error : onError,
		beforeSend : onBeforeSend,
...
function saveForm() {
	...
	$.ajax({
		type : "POST",
		url : '/projects/mng/save?_csrf=' + csrf,
