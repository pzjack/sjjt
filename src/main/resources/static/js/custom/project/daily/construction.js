...
function loadData(pageIndex, pageSize) {
	$.ajax({
		type : "POST",
		url : '/project/daily/contruct/list?pageIndex=' + pageIndex + '&pageSize=' + pageSize
				+ '&_csrf=' + csrf,
...
function openAddWindow() {
	openWindow("新增信息", '/project/daily/contruct/formInit', 470, 380);
}
...
function openEditWindow(id) {
	var href = '/project/daily/contruct/findOne?id=' + id;
	openWindow("编辑信息", href, 470, 380);
}
...
function doDelete(param) {
	$.ajax({
		type : "POST",
		url : '/project/daily/contruct/delete?_csrf=' + csrf,
		data : param,
		success : afterDelete,
		error : onError,
		beforeSend : onBeforeSend,
...
function saveForm() {
	...
	$.ajax({
		type : "POST",
		url : '/project/daily/contruct/save?_csrf=' + csrf,
