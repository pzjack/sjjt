...
opts=$.extend({},$.messager.defaults,{content:"<div class=\"messager-icon messager-question\"></div>"+"<div>"+opts.msg+"</div>"+"<br/>"+"<div style=\"clear:both;\"/>"+"<div><input class=\"messager-input \" type=\"text\" maxlength=\"300\"/></div>",buttons:[{text:$.messager.defaults.ok,onClick:function(){
win.window("close");
...
sele._outerHeight(body._outerHeight()+1);//modify by 段志芳 on 2015-07-13 自定义年月控件时，月份panel无法将日期panel遮挡完全，因此将高度加1
...
var tableWidth=	dc.view.width()-opts.scrollbarSize;
var _623=0;
//小于画面中设定的table宽度时，按照设定的table宽度计算列宽，使table出现滚动条 段志芳 2015-07-08 修改
if(tableWidth<opts.tableWidth){
	var _9=opts.scrollbarSize||0;
	var v=$.trim(String(col.width||""));
	var _a=v.substr(v.length-1,1);
	if(_a=="%"){
	v=parseInt(v.substr(0,v.length-1));
	_623=Math.floor((opts.tableWidth-_9)*v/100);
	}
}else{
	//正常处理分支
	_623=$.parser.parseValue("width",col.width,dc.view,opts.scrollbarSize)-col.deltaWidth;
}
//var _623=$.parser.parseValue("width",col.width,dc.view,opts.scrollbarSize)-col.deltaWidth;
if(_623>0){
	//有checkbox列，且有序号列时，需要将最后一列宽度减去50，防止出现滚动条  段志芳 2015-06-25 修改
	if(opts.checkbox){
		if(_621.length==i+1 && opts.rownumbers){
			col.boxWidth=_623-50;
		}else{
			col.boxWidth=_623;
		}	
	}else{
		//无checkbox列时，有序号列时，需特殊处理，需要将最后一列宽度减去25。 段志芳 2015-05-14 修改	
		if(_621.length==i && opts.rownumbers){
			col.boxWidth=_623-25;
		}else{
			col.boxWidth=_623;
		}
	}
...
//翻页时，不清除已选项 Modify by 段志芳  on 2015-07-11
...
//s中保存的是已选项的id，为在翻页时保留已选项名称，改为从text中取值 Modify by 段志芳 on 2015-07-11
//$(_9ce).combo("setText",s);
$(_9ce).combo("setText",$(_9ce).combo("getText"));
}
}
